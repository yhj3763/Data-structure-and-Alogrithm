...
//It is linked list with dummy node
//Although it does not give reasonable change in performance, but it helps us make code clearer 
//When there are a lot of small lists, it will be bad choice to use becauses there will be a lot of dummy node for each list
//Head->Dummy->Null
...
//In list that has dummy node, we do not have to deal with head node because head node always indicates dummy node.

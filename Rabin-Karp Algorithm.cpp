...
//When we want to move to next character, we need to use 
//sum = 2 * (sum - originalstring[charachter we want to remove] * power(usually 2^size of target-1)+originalstring[character we want to add])

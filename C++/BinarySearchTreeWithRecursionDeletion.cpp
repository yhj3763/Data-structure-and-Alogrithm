...
public:
	...
	Node* removerecursion(Node* proot, Data target);; //It is remove function by using recursion
...
//It is remove function with recursion
//It gets root as parameter to use it for recursion
Node* BStree::removerecursion(Node* proot, Data target) {

	//if there is no value in tree, return NULL
	if (proot == NULL) {
		return NULL;
	}
	//if target is smaller than data in root, goes to left node of root
	else if (target < proot->data) {
		proot->left = removerecursion(proot->left, target);
	}
	//if target is bigger than data in root, goes to right node of root
	else if (target > proot->data) {
		proot->right = removerecursion(proot->right, target);
	}
	//Now, we found place for value we want to remove
	else {

		//When there is no child for target, delete root and put NULL value into root and return it
		//Because we used recursion to reach here, root indicates target we want to remove now.
		if (proot->left == NULL && proot->right == NULL) {
			delete proot;
			proot = NULL;
			return proot;
		}
		//When there is only one child for target, make temp node and store right value into root and delete temp and return 
		...
		//When there is only one child for target, make temp node and store left value into root and delete temp and return 
		...
		//When there are two child for target, we need to find minimum value in the right side of target
		else {

			//Put right side of target to find minimum
			Node* min = proot->right;

			//Repeat until it finds replacement for target
			while (min->left != NULL) {
				min = min->left;
			}

			//Store replacement to temp
			//Put remplacement data into target data
			//replacement can also have right child, so we need to use removerecursion function again to deal with the case
			Node* temp = min;
			proot->data = temp->data;
			proot->right = removerecursion(proot->right, temp->data);
		}
...
int main() {
	...
	bs.removerecursion(bs.getroot(), 8);

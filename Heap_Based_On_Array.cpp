...
private:
	...
	Heapelem harr[ARRAY_LENGTH];			//Array for heap
...
int Heap::gethigherprior(int idx) {
	...
	//For other case, parent has both children, so we need to compare them
		...
		if (harr[lchild].prior < harr[rchild].prior) {
			return lchild;
		}
		else {
			return rchild;
...
void Heap::insert(Data value, int prior) {
	...
	while (idx != 1) {
		...
		if (prior < harr[getparentidx(idx)].prior) {
...
Data Heap::remove() {
	...
	Data rdata = harr[1].data;
	Heapelem lastelm = harr[numofdata];
	...
	while (childidx = gethigherprior(parentidx)) {
		...
		if (lastelm.prior <= harr[childidx].prior) {
			break;
		}
